...
import type { DefectEventType, FieldDefectSeverity, FieldDefectSource, FieldDefectStatus, Prisma } from "@plantx/db/client"
...
export async function getSuppliersForField() {
  ...
  const supplierIds = new Set<string>([
    ...
    ...fieldDefectSupplierIds.map((d) => d.supplierId).filter((s): s is string => s !== null),
...
export async function addFieldDefectComment(id: string, content: string) {
  ...
  const canWrite = isOem
    ...
    : isSupplier && isEditorRole(session.user.role)
...
export async function escalateFieldDefect(id: string, reason: string) {
  ...
  const oemUsers = await prisma.user.findMany({
    where: { companyId: session.user.companyId, role: { in: ["ADMIN", "EDITOR"] } },
    select: { id: true },
  })
