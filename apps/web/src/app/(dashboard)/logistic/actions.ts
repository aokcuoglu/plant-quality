...
import { createOrderLinesAndUnits, type OrderLineInput } from "./flow-actions"
...
export async function createLogisticOrder(formData: FormData) {
  ...
  if (!canSalesExport(session.user.role)) {
  ...
  const orderLinesRaw = String(formData.get("orderLines") ?? "")
  let orderLines: OrderLineInput[] = []
  try {
    const parsed: unknown = JSON.parse(orderLinesRaw)
    if (Array.isArray(parsed)) orderLines = parsed.filter((line): line is OrderLineInput => Boolean(line) && typeof line === "object" && typeof (line as { vehicleModelId?: unknown }).vehicleModelId === "string" && Number((line as { quantity?: unknown }).quantity) === 1 && (typeof (line as { identifier?: unknown }).identifier === "undefined" || typeof (line as { identifier?: unknown }).identifier === "string"))
  } catch {
    orderLines = []
  }
  if (!orderLines.length) throw new Error("At least one valid order line is required")
  const selectedModels = await prisma.logisticVehicleModel.findMany({ where: { companyId, id: { in: orderLines.map((line) => line.vehicleModelId) }, active: true }, include: { group: true } })
  if (selectedModels.length !== new Set(orderLines.map((line) => line.vehicleModelId)).size) throw new Error("Vehicle model not found")
  const firstModel = selectedModels.find((model) => model.id === orderLines[0].vehicleModelId)!
  const vehicleModel = firstModel.name
  const quantity = orderLines.reduce((total, line) => total + line.quantity, 0)
  ...
  const vehicleVariant = orderLines[0].variant?.trim() || null
  ...
  const vehicleType = (["BUS", "MIDIBUS", "TRUCK", "LIGHT_TRUCK"].includes(firstModel.group.code) ? firstModel.group.code : "OTHER") as LogisticOrderVehicleType
  const powertrain = orderLines[0].powertrain || "DIESEL"
  const priority = orderLines[0].priority || "NORMAL"
  ...
  const lineResult = await createOrderLinesAndUnits(order.id, orderLines)
  if (!lineResult.success) {
    await prisma.plantLogisticOrder.delete({ where: { id: order.id } })
...
export async function updateLogisticOrderPlanning(orderId: string, formData: FormData) {
  ...
  if (!canProduction(session.user.role)) {
    return { error: "Bu işlem için yetkiniz yok" }
  }
