...
function stageDenied(role: Role, _stage: VehicleStage): string | null {
  return isEditorRole(role) ? null : "Bu aşama için yetkiniz yok"
}
...
export async function moveOrderToStage(
  ...
  const denied = stageDenied(role, targetStage)
...
export async function requestDispatch(orderId: string): Promise<StageActionResult> {
  const { companyId, userId, role } = await authGate()
  if (!canDelivery(role)) return { success: false, error: "Bu işlem için yetkiniz yok" }
  const order = await loadOrder(orderId, companyId)
  if (!order) return { success: false, error: "Order not found" }
