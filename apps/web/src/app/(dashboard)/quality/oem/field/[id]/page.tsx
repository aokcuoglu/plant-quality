...
export default async function OemFieldDetailPage({
  ...
  const canEdit = isEditorRole(session.user.role) && ["DRAFT", "OPEN", "UNDER_REVIEW", "SUPPLIER_ASSIGNED"].includes(fd.status)
  const canConvert = isEditorRole(session.user.role) && !fd.linkedDefectId && fd.supplierId && ["OPEN", "UNDER_REVIEW", "SUPPLIER_ASSIGNED"].includes(fd.status)
  const canChangeStatus = isEditorRole(session.user.role)
  const canAssign = isEditorRole(session.user.role) && ["DRAFT", "OPEN", "UNDER_REVIEW"].includes(fd.status)
