...
export default async function SupplierEscalationsPage({
  searchParams,
}: {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Escalations"
        description="Escalated field defects assigned to your company"
      />
      ...
      <div className="flex flex-wrap items-center gap-2">
        {LEVEL_FILTERS.map((lf) => (
          <Link
            key={lf.value}
            href={lf.value ? `/quality/supplier/escalations?filter=${lf.value}` : "/quality/supplier/escalations"}
              ...
              {escalations.map((esc) => (
                <tr key={esc.id} className="border-b transition-colors hover:bg-muted/50">
                  <td className="p-3 align-middle">
                    <Link href={`/quality/supplier/field/${esc.entityId}`} className="font-medium text-foreground hover:underline">
