...
import type { FmeaStatus, FmeaActionStatus } from "@plantx/db/client"
...
export default async function SupplierFmeaDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const isSupplierAdminOrQe = isEditorRole(session.user.role)
            ...
            <Table className="w-full text-xs">
              <TableHeader>
                <TableRow className="border-b border-border bg-muted/50">
                  ...
                  <TableHead className="px-2 py-2 text-left text-[10px] font-medium uppercase tracking-wider text-muted-foreground min-w-[100px]">Supplier Comment</TableHead>
              ...
              <TableBody className="divide-y divide-border">
                {rows.map((row, i) => (
                  <TableRow key={row.id ?? i} className={Number.isFinite(row.rpn) && row.rpn >= 200 ? "bg-destructive/5" : Number.isFinite(row.rpn) && row.rpn >= 100 ? "bg-destructive/5" : ""}>
                    ...
                    <TableCell className="max-w-[150px] truncate px-2 py-2 text-muted-foreground">{row.supplierComment || "—"}</TableCell>
                    <TableCell className="max-w-[150px] truncate px-2 py-2 text-muted-foreground">{row.oemComment || "—"}</TableCell>
                  </TableRow>
                ))}
                {rows.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={fmea.fmeaType === "PROCESS" ? 17 : 16} className="px-4 py-8 text-center text-sm text-muted-foreground">
                      No rows added yet
                    </TableCell>
                  </TableRow>
                )}
