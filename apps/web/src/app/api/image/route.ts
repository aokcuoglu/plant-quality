...
export async function GET(req: NextRequest) {
  ...
  if (key.startsWith("defects/")) {
    ...
    if (parts.length >= 3) {
      ...
      // ImageUploader keys are scoped to the uploading tenant:
      // "defects/<companyId>/<uuid>.<ext>". Allow access to the owning company only.
      if (!parentOwnerOk && parts[1] === session.user.companyId) {
        parentOwnerOk = true
      }
    }
