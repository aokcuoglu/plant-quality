...
export function getModuleStatus(
  ...
  if (entry.moduleKey) {
    const hasAccess = checkModuleAccess(entry.moduleKey, companyId, companyType, undefined, undefined, companyModules)
...
export function checkModuleAccess(
  ...
  // Dealers/distributors only consume the external portal; they get no backend modules.
  if (companyType === "DEALER" || companyType === "DISTRIBUTOR") return false

  const entitlement = MODULE_ENTITLEMENTS[moduleKey]
  if (!entitlement) return false
  // Supplier companies are only entitled to modules marked as supplierAccess.
  if (companyType === "SUPPLIER" && !entitlement.supplierAccess) return false

  // Explicit per-user module grants take priority over the role default and the
  // company subscription. Admins can grant a module even if the company owns it.
  if (userModules && userModules.length > 0) {
    return userModules.includes(moduleKey)
  }

  // A user gets a module only if their company is subscribed to it (falls back
  // to the built-in entitlement map when the subscription is not resolved).
  const modules = companyModules && companyModules.length > 0
    ? companyModules
...
export function checkFeatureAccess(
  ...
  if (gate.module) {
    ...
    const hasModuleAccess = companyId
      ? checkModuleAccess(gate.module, companyId, companyType, undefined, undefined, companyModules)
...
export function canUseFeature(
  ...
  return checkFeatureAccess(plan, companyType, featureKey, companyId, companyModules).allowed
}
