import type { Plan } from "@plantx/db/client"
...
export function getSessionPlanInfo(session: {
  ...
  const companyModules = session?.user?.companyModules ?? null
  return { plan, companyType, companyId, companyModules }
}

export function requireModule(
  session: { user?: { plan?: Plan | string | null; companyType?: string | null; companyId?: string | null; role?: string | null; modules?: string[] | null; companyModules?: string[] | null } } | null,
  moduleKey: ModuleKey
): { allowed: boolean; reason: string | null } {
  const { companyId, companyType, companyModules } = getSessionPlanInfo(session)
  const hasAccess = checkModuleAccess(moduleKey, companyId, companyType, session?.user?.role, session?.user?.modules, companyModules)
  if (hasAccess) return { allowed: true, reason: null }
  return { allowed: false, reason: `The ${moduleKey.replace("_MODULE", "").replace(/_/g, " ")} module is not included in your company subscription.` }
}

export function requireFeature(
  session: { user?: { plan?: Plan | string | null; companyType?: string | null; companyId?: string | null; companyModules?: string[] | null } } | null,
  featureKey: FeatureKey
): { allowed: boolean; reason: string | null } {
  const { plan, companyType, companyId, companyModules } = getSessionPlanInfo(session)
  const allowed = canUseFeature(plan, companyType, featureKey, companyId, companyModules)
...
export function isFeatureGatedNav(href: string): FeatureKey | null {
  const gatedNav: Record<string, FeatureKey> = {
    ...
    "/logistic/board": "PLANT_LOGISTIC",
    "/logistic/plan-sheets": "PLANT_LOGISTIC",
    "/logistic/dispatches": "PLANT_LOGISTIC",
