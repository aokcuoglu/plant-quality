...
import type { DefectStatus, NotificationType, FieldDefectStatus } from "@plantx/db/client"
  ...
  for (const fd of fieldDefects) {
    ...
    if (slaStatus === "due-soon") {
      ...
      const oemUsers = await prisma.user.findMany({
        where: { companyId: fd.oemId, role: { in: ["ADMIN", "EDITOR"] } },
        select: { id: true },
      })
      ...
      if (fd.supplierId) {
        const supplierUsers = await prisma.user.findMany({
          where: { companyId: fd.supplierId, role: { in: ["ADMIN", "EDITOR"] } },
          select: { id: true },
        })
