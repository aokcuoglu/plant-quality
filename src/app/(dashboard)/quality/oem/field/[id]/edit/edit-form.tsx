    ...
    <form onSubmit={(e) => startTransition(() => handleSubmit(e))} className="space-y-5">
      ...
      <div className="border-t pt-5">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <div className="space-y-2"><label htmlFor="category" className="text-sm font-medium">Category</label><input id="category" name="category" type="text" defaultValue={fieldDefect.category ?? ""} maxLength={100} placeholder="e.g., Electrical" className="flex h-10 w-full rounded-lg border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2" /></div>
          <div className="space-y-2"><label htmlFor="subcategory" className="text-sm font-medium">Subcategory</label><input id="subcategory" name="subcategory" type="text" defaultValue={fieldDefect.subcategory ?? ""} maxLength={100} placeholder="e.g., Wiring Harness" className="flex h-10 w-full rounded-lg border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2" /></div>
          <div className="space-y-2"><label htmlFor="probableArea" className="text-sm font-medium">Probable Area</label><input id="probableArea" name="probableArea" type="text" defaultValue={fieldDefect.probableArea ?? ""} maxLength={100} placeholder="e.g., Front Left Door" className="flex h-10 w-full rounded-lg border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2" /></div>
        </div>
        <p className="text-xs text-muted-foreground mt-2">Leave fields empty to clear. Max 100 characters each.</p>
      </div>
      ...
      <div className="flex items-center gap-3 pt-2">
        ...
        <Link href={`/quality/oem/field/${fieldDefect.id}`} className="inline-flex h-8 items-center justify-center gap-1.5 rounded-lg border border-border bg-background px-3 text-sm font-medium text-muted-foreground hover:bg-muted hover:text-foreground transition-colors">
          Cancel
        </Link>
      </div>
    </form>
