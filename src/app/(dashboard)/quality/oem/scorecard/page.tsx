...
function ScoreBar({ score }: { score: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-sm font-bold tabular-nums w-8 text-right">{safeScore}</span>
    </div>
  )
}
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Deterministic supplier quality scoring — field defects, 8D, IQC, PPAP, FMEA, and SLA signals"
      />
      ...
      <p className="text-xs text-muted-foreground">Scores are deterministic, derived from your workflow data. No AI-generated content. Penalties cap per category.</p>

      <div className="rounded-lg border bg-card">
        <div className="px-4 py-3 border-b border-border flex items-center justify-between">
          <h2 className="text-sm font-semibold">Supplier Rankings</h2>
    ...
    <tr className="group hover:bg-muted/50">
      <td className="px-4 py-3">
        ...
        <div className="text-xs text-muted-foreground mt-0.5">
          {supplier.openIssuesCount} open issues
        </div>
      </td>
