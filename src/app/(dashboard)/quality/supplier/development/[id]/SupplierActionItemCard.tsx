...
import { ACTION_STATUS_CONFIG } from "@/lib/supplier-development/client"
import { useState } from "react"

export function SupplierActionItemCard({ item, planId, isReadOnly, canSubmit }: { item: DevActionItemDetail; planId: string; isReadOnly: boolean; canSubmit: boolean }) {
  ...
  const handleStatusUpdate = async (newStatus: string) => {
    ...
    await updateSupplierActionItem(formData)
  ...
  const isOverdue = item.dueDate && new Date(item.dueDate) < new Date() && item.status !== "COMPLETED" && item.status !== "ACCEPTED" && item.status !== "CANCELLED"
  ...
  return (
    <div className="rounded-md border border-border p-3">
      ...
      {isReadOnly && item.supplierResponse && (
