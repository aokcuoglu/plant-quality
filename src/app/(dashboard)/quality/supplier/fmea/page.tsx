    ...
    <div className="space-y-6">
      ...
      {fmeas.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-border bg-card p-12 text-center">
          ...
          <p className="mt-1 text-xs text-muted-foreground">FMEA analyses will appear here when created by your OEM</p>
        </div>
      ) : (
              ...
              {fmeas.map((f) => (
                <tr key={f.id} className="transition-colors hover:bg-muted/50">
                  <td className="px-4 py-3">
                    <Link href={`/quality/supplier/fmea/${f.id}`} className="font-medium text-foreground hover:text-emerald-400">{f.title}</Link>
                  </td>
