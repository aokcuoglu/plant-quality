    ...
    <div className="space-y-6">
      ...
      {reports.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-border bg-card p-12 text-center">
          ...
          <p className="mt-1 text-xs text-muted-foreground">Incoming quality inspection reports will appear here</p>
        </div>
      ) : (
              ...
              {reports.map((r) => (
                <tr key={r.id} className="transition-colors hover:bg-muted/50">
                  <td className="px-4 py-3">
                    <Link href={`/quality/supplier/iqc/${r.id}`} className="font-medium text-foreground hover:text-emerald-400">{r.lotNumber}</Link>
                  </td>
                  <td className="px-4 py-3 text-muted-foreground">{r.partNumber}</td>
                  <td className="px-4 py-3 text-muted-foreground">{r.oem.name}</td>
