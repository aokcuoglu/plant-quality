...
import { checkModuleAccess, type FeatureKey, type ModuleKey, MODULE_ENTITLEMENTS } from "@/lib/billing/features"

type PlantXModule = "quality" | "logistic" | null

const MODULE_MAP: Record<Exclude<PlantXModule, null>, ModuleKey> = {
...
export function AppSwitcher({ currentModule, userPlan = "FREE", userCompanyType = "OEM", userCompanyId = "" }: AppSwitcherProps) {
      ...
      >
        <DropdownMenuGroup className="space-y-0.5">
          ...
          {apps.map((app) => {
            ...
            if (app.module) {
              const moduleKey = MODULE_MAP[app.module]
              const hasModule = moduleKey ? checkModuleAccess(moduleKey, companyId, companyType) : false
              const isVisible = app.supplierVisible || companyType !== "SUPPLIER"
              if (!isVisible) {
                return null
              }
              if (!hasModule) {
                const entitlement = moduleKey ? MODULE_ENTITLEMENTS[moduleKey] : null
                return (
                  <DropdownMenuItem
                    key={app.id}
                    disabled
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="truncate text-xs text-muted-foreground/50">{entitlement?.description ?? app.description}</p>
                    </div>
                  </DropdownMenuItem>
                )
              }
